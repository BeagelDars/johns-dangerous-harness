#!/usr/bin/env python3
"""
Autonomous Cloud Scraper: hacker_news
Target: https://news.ycombinator.com
Criteria: ai agent model
Scheduled via GitHub Actions (Cron: 0 * * * *)
"""
import os
import sys
import json
...
CRITERIA = "ai agent model"
DATA_FILE = os.path.join(os.path.dirname(__file__), "data", "hacker_news_results.json")
...
def run_scrape():
    print(f"[Scraper] Starting scrape for {TARGET_URL} at {datetime.utcnow().isoformat()}Z...")
        ...
        if clean_text and len(clean_text) > 8 and not href.startswith('#') and not href.startswith('javascript:'):
            ...
            if CRITERIA:
                ...
                if any(kw in clean_text.lower() for kw in keywords):
                    items.append({"title": clean_text, "url": full_url, "scraped_at": datetime.utcnow().isoformat()})
            else:
                items.append({"title": clean_text, "url": full_url, "scraped_at": datetime.utcnow().isoformat()})
